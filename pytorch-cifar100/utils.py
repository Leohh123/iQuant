...
def quantize(module: nn.Module, bit, fn_filter=None):
    print(f'Bit: {bit}')
    total_quantized, total_param = 0, 0
    for k, v in module.state_dict().items():
        num_param = torch.numel(v)
        total_param += num_param
        if (fn_filter is None or fn_filter(k, v)) and v is not None:
            total_quantized += num_param
            print(f'Quantize: {k}, #params: {num_param}')
    ...
    print(f'Quantization rate: {total_quantized * 100. / total_param:.2f}')
...
def quant_thresh_and_rate(module: nn.Module, target_rate=0.9):
    nums = sorted([torch.numel(v)
                  for k, v in module.state_dict().items()], reverse=True)
    cnt, total = 0, sum(nums)
    thresh = None
    for n in nums:
        if thresh is None:
            cnt += n
            current_rate = cnt / total
...
def compute_mean_std(cifar100_dataset):
    ...
    data_r = np.dstack([cifar100_dataset[i][1][:, :, 0]
                        for i in range(len(cifar100_dataset))])
    data_g = np.dstack([cifar100_dataset[i][1][:, :, 1]
                        for i in range(len(cifar100_dataset))])
    data_b = np.dstack([cifar100_dataset[i][1][:, :, 2]
                        for i in range(len(cifar100_dataset))])
